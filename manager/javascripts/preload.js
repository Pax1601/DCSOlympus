...
function failUpdate() {
  showErrorPopup(
    `<div class='main-message'>An error has occurred while updating Olympus. </div><div class='sub-message'> Please delete Olympus and update it manually. A browser window will open automatically on the download page. <br><br> You can find more info in ${path.join(
      __dirname,
      "..",
      "manager.log"
    )}</div>`,
...
const ipc = {
  render: {
    /* From render to main. */
    send: ["window:minimize", "window:maximize", "window:restore", "window:close"],
    /* From main to render. */
    receive: ["event:maximized", "event:unmaximized"],
