...
import { BLUE_COMMANDER, DataIndexes, GAME_MASTER, IADSDensities, OlympusState, RED_COMMANDER, UnitControlSubState } from "../constants/constants";
import {
...
export class UnitsManager {
        ...
        for (let ID in this.#units) {
            if (this.#units[ID].getSelected()) {
                ...
                unitPath.forEach((latlng: LatLng) => {
                    if (pathMarkersCoordinates.every((coord: LatLng) => coord.lat != latlng.lat && coord.lng != latlng.lng))
                        pathMarkersCoordinates.push(latlng);
                });
            }
        }
    ...
    /** Set a specific posture to all the selected units
     *
     * @param posture Value to set, see constants for acceptable values
     * @param units (Optional) Array of units to apply the control to. If not provided, the operation will be completed on all selected units.
     */

    setPosture(posture: number, units: Unit[] | null = null, onExecution: () => void = () => {}) {
        if (units === null) units = this.getSelectedUnits();
        units = units.filter((unit) => !unit.getHuman());
        let callback = (units: Unit[]) => {
            onExecution();
            units.forEach((unit: Unit) => unit.setPosture(posture));
            this.#showActionMessage(units, `posture set to ${posture}`);
        };
        if (getApp().getMap().getOptions().protectDCSUnits && !units.every((unit) => unit.isControlledByOlympus())) {
            getApp().setState(OlympusState.UNIT_CONTROL, UnitControlSubState.PROTECTION);
            this.#protectionCallback = callback;
