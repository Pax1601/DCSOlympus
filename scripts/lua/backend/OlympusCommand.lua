...
-- coalition: (string)
-- country: (string)
-- groupName: (string, optional) if not provided it will be generated by Olympus
...
function Olympus.spawnUnits(spawnTable, requestHash) 
	...
	local vars = 
	{
		...
		category = category,
		route = route,
		name = spawnTable.groupName or "Olympus-" .. Olympus.unitCounter,
		task = 'CAP'
	}
...
function Olympus.generateAirUnitsTable(units)
	...
	for idx, unit in pairs(units) do
		...
		unitsTable[#unitsTable + 1] = 
		{
			...
			["callsign"] = { [1] = 1, [2] = 1, [3] = 1, ["name"] = unit.name or "Olympus" .. Olympus.unitCounter.. "-" .. #unitsTable + 1 },
			["name"] = unit.name or "Olympus-" .. Olympus.unitCounter .. "-" .. #unitsTable + 1,
