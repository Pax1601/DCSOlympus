...
import { CommandModeOptions, LoadoutBlueprint, MissionData, OlympusConfig, ServerStatus, SessionData, SpawnRequestTable, UnitBlueprint, UnitData } from "./interfaces";
...
export class AppStateChangedEvent {
    static on(
        callback: (state: OlympusState, subState: OlympusSubState, previousState: OlympusState, previousSubState: OlympusSubState) => void,
        singleShot = false
    ) {
        document.addEventListener(
            this.name,
            (ev: CustomEventInit) => {
                callback(ev.detail.state, ev.detail.subState, ev.detail.previousState, ev.detail.previousSubState);
            },
            { once: singleShot }
        );
    }

    static dispatch(state: OlympusState, subState: OlympusSubState, previousState: OlympusState, previousSubState: OlympusSubState) {
        const detail = { state, subState, previousState, previousSubState };
...
export class SetLoadoutWizardBlueprintEvent {
    static on(callback: (blueprint: UnitBlueprint) => void, singleShot = false) {
        document.addEventListener(
            this.name,
            (ev: CustomEventInit) => {
                callback(ev.detail.blueprint);
            },
            { once: singleShot }
        );
    }

    static dispatch(blueprint: UnitBlueprint) {
        document.dispatchEvent(new CustomEvent(this.name, { detail: { blueprint } }));
        if (DEBUG) console.log(`Event ${this.name} dispatched`);
    }
}

export class CustomLoadoutsUpdatedEvent {
    static on(callback: (unitName: string, loadout: LoadoutBlueprint) => void, singleShot = false) {
        document.addEventListener(
            this.name,
            (ev: CustomEventInit) => {
                callback(ev.detail.unitName, ev.detail.loadout);
            },
            { once: singleShot }
        );
    }
    static dispatch(unitName: string, loadout: LoadoutBlueprint) {
        document.dispatchEvent(new CustomEvent(this.name, { detail: { unitName, loadout } }));
        if (DEBUG) console.log(`Event ${this.name} dispatched`);
    }
}
