...
export class PlaybackPipeline {
    ...
    constructor() {
        ...
        let bandpassLow = new Filter(audioContext, "bandpass", 520, 0.25);
        bandpassLow.setup();

        // Bandpass filter for high frequency cutoff at 4130 Hz same as SRS
        let bandpassHigh = new Filter(audioContext, "bandpass", 5000, 0.3);
        bandpassHigh.setup();
	    ...
	    this.#pannerNode.pan.setValueAtTime(0, audioContext.currentTime); */
        ...
        let noise = new Noise(audioContext, 0.02); // Slightly louder static
        noise.init();
    ...
    setEnabled(enabled: boolean) {
        ...
        if (enabled && !this.#enabled) {
            ...
            this.#enabled = false;
            this.#pannerNode?.disconnect(audioContext.destination);
        }
    }
