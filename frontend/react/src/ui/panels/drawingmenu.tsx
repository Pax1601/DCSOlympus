...
import { FaPencil, FaRegCompass, FaXmark } from "react-icons/fa6";
...
import { Circle, Marker, Polygon, Polyline } from "leaflet";
...
export function DrawingMenu(props: { open: boolean; onClose: () => void }) {
    ...
    const [typesSelection, setTypesSelection] = useState({} as { [key: string]: boolean });
    const [erasSelection, setErasSelection] = useState({} as { [key: string]: boolean });
    const [rangesSelection, setRangesSelection] = useState({} as { [key: string]: boolean });
    ...
    let eras = getApp()
        ...
        .getDatabase()
        .getEras()
        .sort((era1, era2) => (ERAS_ORDER[era1 as keyof typeof ERAS_ORDER] > ERAS_ORDER[era2 as keyof typeof ERAS_ORDER] ? 1 : -1));
                        ...
                        <div className="flex justify-between gap-2">
                            ...
                            <div
                                className={`
                                  w-40
                                  ${getDrawingLabelColor(container)}
                                  w-max-40 overflow-hidden text-ellipsis
                                  text-nowrap bg-
                                `}
                            >
                                {container.getName()}
                            ...
                            return (
                                <div className="ml-4 flex justify-start gap-2" key={index}>
                                    ...
                                    <FontAwesomeIcon
                                        ...
                                        `}
                                        onClick={() => {
                                            const layer = drawing.getLayer();

                                            let latlng: L.LatLng | null = null;
                                            let bounds: L.LatLngBounds | null = null;

                                            if (layer instanceof Marker || layer instanceof Circle) {
            ...
            wiki={() => {
                return (
                    ...
                    >
                        ...
                        <div>
                            The drawing menu allows you to create and manage custom drawings, such as polygons and circles, and to generate IADS (Integrated Air
                            Defense System) areas. Moreover, you can manage the visibility and opacity of mission drawings, i.e. drawings from the Mission
                            Editor.
                        </div>
                        <h2 className="my-4 font-bold">Custom drawings and IADS</h2>
                        <div>
                            To create a custom drawing, click on the 'Add polygon' or 'Add circle' buttons, then click on the map to add polygons or to move the
                            drawing. Double-click on the map to finish your creation. You can then edit the drawing by clicking on it. You can also move it up
                            or down in the list, or delete it.
                        </div>
                        <div>
                            You can change the name and the coalition of the area. You can also generate an IADS area by selecting the types, eras, and ranges
                            of units you want to include in the area. You can also set the density and distribution of the IADS. If you check the 'Force
                            coalition appropriate units' box, the IADS will only include units that are appropriate for the coalition of the area (e.g. Hawk
                            SAMs for {""}
              ...
              `}
                        ...
                        <div>
                            The IADS generator will create a random distribution of units in the area, based on the density and distribution you set. Units will
                            be concentrated around cities, and airbases that belong to the selected coalition.
                        </div>
                        <h2 className="my-4 font-bold">Mission drawings</h2>
                        <div>
                            You can manage the visibility and opacity of mission drawings by clicking on the eye icon. Moreover, you can change the opacity of
                            the drawing by using the slider. You can also hide or show all the drawings in a container.
                        </div>
                        <div>
                            You can search for a specific drawing by typing in the search bar. The search is case-insensitive and will match any part of the
                            drawing name.
                        </div>
                        <div>
                            Any change you make is persistent and will be saved for the next time you reload Olympus, as long as the DCS mission was not
                            restarted.
                        </div>
                    </div>
                );
                    ...
                    <div className="flex flex-col gap-2 text-sm text-gray-400">
                            ...
                            {coalitionAreas.map((coalitionArea) => {
                                return (
                                    <div
                                        ...
                                        className={`
                                          flex cursor-pointer content-center
                                          border-l-4 px-4 text-base text-white
                                    ...
                                    >
                                        ...
                                        <FaArrowUp
                                            ...
                                            className={`
                                              my-auto ml-auto rounded-md p-2
                                              text-3xl
                                              hover:bg-white/10
                                            `}
                                        />
                        ...
                        <div className="flex flex-col gap-2 p-6">
                            ...
                            <OlStateButton
                                ...
                                icon={faCircle}
                                checked={false}
                                onClick={() => getApp().setState(OlympusState.DRAW, DrawSubState.DRAW_CIRCLE)}
                            >
                        ...
                        <div>
                            <div className="flex flex-col gap-2 p-6">
                                ...
                                >
                                    <span
                                        className={`
                                          mr-2 px-1 py-1 text-center font-bold
                                          text-olympus-700 text-white
                                        `}
                                    >
                                        <FaPencil />
                                    </span>
            ...
            <div>
                {activeCoalitionArea !== null && (
                    <div className={`flex flex-col gap-4 py-4`}>
                        <div
                            className={`
                              flex flex-col content-center justify-start gap-2
                              px-6 text-gray-200
                            `}
                        >
                            ...
                            <input
                                type="text"
                                className={`
                                  block w-full flex-grow rounded-lg border
                        ...
                        <div
                            className={`
                              flex content-center justify-start gap-4 px-6
                              text-gray-200
                            `}
                        >
                        ...
                        <div
                            className={`
                              ...
                              border-l-olympus-100 bg-olympus-600 p-5
                            `}
                        >
                            <div
                                className={`
                                  border-b-2 border-b-olympus-100 pb-4
                                  text-gray-300
                                `}
                            >
                            ...
                            <div
                                className={`
                                  flex content-center gap-4 text-gray-200
                                `}
                            >
                                <OlCheckbox
