...
import { PickupMarker } from "../map/markers/pickupmarker";
...
export abstract class Unit extends CustomMarker {
    ...
    setDetectionMethods(newDetectionMethods: number[]) {
        if (!this.belongsToCommandedCoalition()) {
            ...
            if (
                ...
                this.getDetectionMethods().some((value) => !newDetectionMethods.includes(value))
            ) {
    ...
    #onMouseUp(e: any) {
        if (
            (getApp().getState() === OlympusState.SPAWN && getApp().getSubState() !== SpawnSubState.NO_SUBSTATE) ||
            getApp().getState() === OlympusState.MEASURE
        )
            return;
    ...
    #updateMarker() {
        ...
        if (element != null) {
            ...
            if (element.querySelector(".unit-callsign"))
                (element.querySelector(".unit-callsign") as HTMLElement).innerText = getApp().getMap().getOptions().showUnitCallsigns
                    ? this.#callsign
                    : this.#unitName;
            ...
            if (element.querySelector(".unit-altitude"))
                (element.querySelector(".unit-altitude") as HTMLElement).innerText =
                    "FL" + zeroAppend(Math.floor(mToFt(this.#position.alt as number) / 100), 3);
            ...
            /* Set operate as */
            element.querySelector(".unit")?.setAttribute("data-operate-as", this.#operateAs);
        }
